...
# 2. تهيئة الذاكرة اللحظية للمحادثة
if "messages" not in st.session_state:
    st.session_state.messages = []

# 3. نظام التصميم الاحترافي (Soft UI)
st.markdown("""
    <style>
...
if prompt := st.chat_input("اسأل لُجّ أي سؤال عن الرخصة المهنية..."):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            if not api_key:
                st.error("خطأ: تأكد من إضافة ANTHROPIC_API_KEY في إعدادات Secrets.")
            else:
